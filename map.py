...
class MapNode:
    def __init__(self, robot_name, robot_list):#输入当前机器人，其他机器人的id list
        ...
        self.map = TopologicalMap(robot_name=robot_name, threshold=0.97) # map就是拓扑地图
        self.vertex_dict = dict() #记录每个机器人有哪几个节点
        self.edge_dict = dict()
        self.relative_position = dict()
        self.relative_orientation = dict()
        self.meeted_robot = list()
        for item in robot_list: # 字典key是robot i ，value是所需要保存的点、边、相对位姿
        ...
        normalize = transforms.Normalize( #对输入做一个标准化，减去平均值除以方差
    ...
    def get_move_goal(self, robot_name, last_pose, nextmove, basic_length=4)-> MoveBaseGoal():
        goal_message = MoveBaseGoal() #这一部分只是利用nextmove获取了所需要的下一个位姿，nextmove应该是一个角度参数，代表往这一个方向走一个步长
        ...
        goal = np.array([x, y])
        pose.x = x*np.cos(theta)-y*np.sin(theta) + self.tf_transform[0]#还原到世界坐标
    ...
    def get_goal_marker(self, robot_name, last_pose, nextmove, basic_length=4) -> PoseStamped():
        ...
        euler = R.from_quat(self.rotation).as_euler('xyz', degrees=True)[2]
        orientation = R.from_euler('z', nextmove+euler, degrees=True).as_quat() #只绕Z轴旋转一个角度
        ...
        pose = Point()
        nextmove = math.radians(nextmove)
        theta = math.radians(euler) # 目前机器人在世界坐标系下转角
        x = last_pose.x + basic_length * np.cos(nextmove)#位置变化量
        y = last_pose.y + basic_length * np.sin(nextmove)
    ...
    def map_odom_callback(self, data):
        #更新自己的pose，map到odom的转移矩阵
        self.pose = data.pose
        if self.init_map_angle_ready == 0:#更新map的信息
            ...
            print('map receive!', 'map angle = ', self.map_angle)
    ...
    def loop_detect_callback(self, data):
        ...
        if self.detect_loop:
            for i in range(len(self.map.vertex)):
                ...
                for j in range(len(vertex.navigableDirection)-1, -1, -1):#len-1, len -2, ... ,0
                    unexplored = vertex.frontierPoints[j]
                    if np.sqrt(np.sum(np.square(position-unexplored))) < 3:
                        meeted.append(j)
                for index in meeted:
                    del self.map.vertex[i].navigableDirection[index]#都是需要初始化的
                    ...
                    self.unexplore_direction_pub.publish(ud_message)#发布未探索方向
            rr = 150
            for i in range(len(self.map.vertex)):
                vertex = self.map.vertex[i]
                vertex_position = np.array([vertex.pose.pose.position.x, vertex.pose.pose.position.y]) #节点位置
                if vertex.navigableDirection:
                    if np.sqrt(np.sum(np.square(position-vertex_position))) < 100:#和需要探索的边界点小于100
                        location = [0, 0]
                        ...
                        self.map.vertex[i].localMap = temp_map #生成局部地图
                        old_ud = self.map.vertex[i].navigableDirection
                        new_ud = self.map.vertex[i].navigableDirection
                        delete_list = []
                        for j in range(len(old_ud)):#这部分好像old和new完全相同，所以两个for循环一定会把delete_list设成0
                            not_deleted = 1
                            for uds in new_ud:
        ...
        if len(self.map.vertex) != 0:
            topomap_message = TopomapToMessage(self.map) # 把拓扑地图做成消息发送出去
            self.topomap_pub.publish(topomap_message)#发布拓扑地图
    ...
    def create_panoramic_callback(self, image1, image2, image3, image4):#合成一张全景图片然后发布
        ...
        cv2.imwrite("/home/song/debug/panormaic.jpg", cv2.cvtColor(self.panoramic_view, cv2.COLOR_BGR2RGB))
        # cv2.imwrite("/home/song/debug/1.jpg", cv2.cvtColor(img1, cv2.COLOR_BGR2RGB))
        # cv2.imwrite("/home/song/debug/3.jpg", cv2.cvtColor(img3, cv2.COLOR_BGR2RGB))
    ...
    def map_panoramic_callback(self, panoramic):# start robot exploration
        start_msg = String()
        ...
        feature = cal_feature(self.net, panoramic_view, self.transform, self.network_gpu) #计算描述子, 一个向量
        current_pose = self.pose #pose is updated by odom
        vertex = Vertex(robot_name, id=-1, pose=current_pose, descriptor=feature) #id = -1
        self.last_vertex, self.current_node, matched_flag = self.map.add(vertex, self.last_vertex, self.current_node)
        #current node 就是当前匹配到的点或者当前添加的点
        if matched_flag==0:
            ...
            self.map.vertex[-1].localMap = localMap #最后一个节点形成时的地图
            self.detect_loop = 0
            ...
            self.goal_pub.publish(goal_marker)#发布目标的marker
            ...
            if len(navigableDirection) == 0:
                ...
                for i in range(len(self.map.vertex[picked_vertex_id].frontierPoints)):
                    ...
                    dis_tmp = np.sqrt(np.sum(np.square(position_tmp-self.map.center))) #边界点和地图中心的距离？
                    dis_scores[i] += epos * dis_tmp
                    for j in range(len(self.meeted_robot)):# meet robot定义？
        ...
        if self.no_place_to_go: #如果没有下一个点情况
            find_a_goal = 0
            position = self.pose.pose.position
            position = np.array([position.x, position.y])
            distance_list = []
            ...
            while(distance_list):
                min_dis = min(distance_list) # 与每个节点最短距离
                index = distance_list.index(min_dis)
                if len(self.map.vertex[index].navigableDirection) != 0:#最短距离存在一个待探索的方向
                    ...
                    if len(self.map.vertex[index].navigableDirection)!=0:
                        ...
                        self.unexplore_direction_pub.publish(ud_message)#直接去探索这个方向，需要删除对应list中元素
    ...
    def map_grid_callback(self, data):
        ...
        try:
            ...
            if robot_name == 'robot1':
                ...
                # cv2.imwrite("/home/master/debug/map.jpg", temp)
                # cv2.imwrite("/home/master/debug/globalmap.jpg", self.global_map)
    ...
    def topomap_callback(self, topomap_message):#融合两个topomap
        Topomap = MessageToTopomap(topomap_message)
        matched_vertex = list()
        vertex_pair = dict()
        if Topomap.vertex[0].robot_name in self.meeted_robot:# already meet this robot
            for vertex in Topomap.vertex:
                if vertex.robot_name!=robot_name:# vertex from other robot exist
                    # this only suit robot team with max size 3
                    # 这里还需要再看一下，感觉有点奇怪
                    if vertex.id not in self.vertex_dict[vertex.robot_name]:
                        # other vertex not received by this robot
                        ...
                        break
        else:# init map merge part 
            for vertex in Topomap.vertex:#对收到拓扑地图中的节点进行分析
                ...
                elif (vertex.id in self.vertex_dict[vertex.robot_name]) or (vertex.robot_name==robot_name):#已经添加过这个id了 or 同一个机器人情况
                    pass
                else:
                    for svertex in self.map.vertex:
                        if svertex.robot_name == vertex.robot_name:# same robot
                            pass
                        else:
                            score = np.dot(vertex.descriptor.T, svertex.descriptor)
                            if score > 0.75:#这里可能有好几个匹配到的结果，假设只有一个（暂时）
                                ...
                                if vertex.robot_name not in self.meeted_robot:
                                    ...
                                    # 将转移矩阵定义为相同点的pose之差
                                    # 其他机器人加上这个relative position才是他在这个机器人坐标系下面的
            ...
            if len(matched_vertex)!=0:#this two map can be matched
                for edge in Topomap.edge:#处理地图融合
                    # edge.link[0][0]is vertex.robot_name| edge.link[0][1]is vertex.id
                    # edge.link[1][0]is vertex.robot_name| edge.link[1][1] is vertex.id
                    if edge.link[0][1] in self.vertex_dict[edge.link[0][0]]:# 后续加入的节点被匹配到了
                        if edge.link[1][1] in self.vertex_dict[edge.link[1][0]] or edge.link[1][0]==robot_name:#本机器人的name
                            pass
                        else:
                            ...
                            self.map.edge_id += 1
                            self.map.edge.append(edge)
                    elif edge.link[1][1] in self.vertex_dict[edge.link[1][0]]:# edge的另一个端点
                ...
                for vertex in Topomap.vertex:
                    if vertex.id in self.vertex_dict[vertex.robot_name]:
                        ...
                        point[0] = (point[0]*number + vertex.pose.pose.position.x) / (number+1)# 有点类似与计算重心
        ...
        for i in range(len(self.map.vertex)):
            ...
            if vertex.robot_name == data.robot_name:
                if vertex.id == data.vertexID:
                    try:
                        del self.map.vertex[i].navigableDirection[data.directionID] #删去导航方向
                    except:
                        pass
