...
from TopoMap_mrtopo import Vertex, Edge, TopologicalMap
...
from utils.topomap_bridge_TM import TopomapToMessage, MessageToTopomap
...
class MapNode:
    def __init__(self, robot_name, robot_list):#输入当前机器人，其他机器人的id list
        ...
        rospy.Subscriber("/allow_move", Int32, self.allow_move_callback, queue_size=1)

    def allow_move_callback(self,data):
    ...
    def topomap_callback(self, topomap_message):#todo
        ...
        if Topomap.vertex[0].robot_name in self.meeted_robot:
            ...
            if len(matched_vertex)!=0:
                for edge in Topomap.edge:#处理地图融合
                    # edge.link[0][0]==vertex.robot_name| edge.link[0][1]==vertex.id
                    # edge.link[1][0]==vertex.robot_name| edge.link[1][1]==vertex.id
