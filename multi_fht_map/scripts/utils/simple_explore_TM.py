...
class RobotNode:
    ...
    def map_grid_callback(self, data):
        ...
        local_grid_map = copy.deepcopy(self.global_map[max(current_loc_pixel[0]-local_range,0):min(current_loc_pixel[0]+local_range,shape[0]), max(current_loc_pixel[1]-local_range,0):min(current_loc_pixel[1]+local_range,shape[1])])
        # print(min(current_loc_pixel[0]-local_range,0),max(current_loc_pixel[0]+local_range,shape[0]), min(current_loc_pixel[1]-local_range,0),max(current_loc_pixel[1]+local_range,shape[1]))
        detected_frontiers = detect_frontier(local_grid_map)
        detected_frontiers_origin = detected_frontiers + np.array([max(current_loc_pixel[1]-local_range,0),max(current_loc_pixel[0]-local_range,0)])
